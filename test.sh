...
# 3. Relay 1
xterm -T "Relay 1 (P:$PORT_RELAY1)" -e "bash -c \"cd moq-relay-ietf && echo 'Working directory: \$(pwd)'; echo 'Starting: Relay 1 on port $PORT_RELAY1'; RUST_LOG=debug cargo run --bin moq-relay-ietf -- --bind '[::]:$PORT_RELAY1' --tls-cert ../dev/localhost.crt --tls-key ../dev/localhost.key --tls-disable-verify --api http://localhost:$PORT_MOQ_API --node https://localhost:$PORT_RELAY1 --dev; exec bash\"" &
sleep $SLEEP_DURATION

# 4. Relay 2
xterm -T "Relay 2 (P:$PORT_RELAY2)" -e "bash -c \"cd moq-relay-ietf && echo 'Working directory: \$(pwd)'; echo 'Starting: Relay 2 on port $PORT_RELAY2'; RUST_LOG=debug cargo run --bin moq-relay-ietf -- --bind '[::]:$PORT_RELAY2' --tls-cert ../dev/localhost.crt --tls-key ../dev/localhost.key --tls-disable-verify --api http://localhost:$PORT_MOQ_API --node https://localhost:$PORT_RELAY2 --dev; exec bash\"" &
sleep $SLEEP_DURATION

# 5. Relay 3
xterm -T "Relay 3 (P:$PORT_RELAY3)" -e "bash -c \"cd moq-relay-ietf && echo 'Working directory: \$(pwd)'; echo 'Starting: Relay 3 on port $PORT_RELAY3'; RUST_LOG=debug cargo run --bin moq-relay-ietf -- --bind '[::]:$PORT_RELAY3' --tls-cert ../dev/localhost.crt --tls-key ../dev/localhost.key --tls-disable-verify --api http://localhost:$PORT_MOQ_API --node https://localhost:$PORT_RELAY3 --dev; exec bash\"" &
sleep $SLEEP_DURATION
